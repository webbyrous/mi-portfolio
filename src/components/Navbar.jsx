    ...
    <nav aria-label='Navegación principal' className='bg-peach backdrop-blur-2xl dark:bg-dark-bg-strong dark:text-grey-medium font-family sticky top-0 lg:relative lg:backdrop-blur-none z-50 transition-colors duration-300 border-b border-grey-medium/50 lg:border-0 dark:border-grey-medium/10'>
      <div className='flex items-center justify-between lg:justify-center py-4 px-6 lg:px-0 lg:gap-8 md:text-lg '>
        ...
        <ul className={`${isOpen ? 'flex' : 'hidden'} 
          ...
          z-50 shadow-2xl lg:shadow-none transition-colors duration-300 lg:dark:bg-transparent`}>
          {navLinks.map((link) => {
            ...
            const isInicio = link.name ==='Inicio';
              ...
              <li key={link.name} onClick={() => setIsOpen(false)} className={`${isInicio ? 'lg:hidden' : ''} w-full lg:w-auto text-center lg:py-1 inline-block transition-all duration-300 rounded-lg px-3 py-1 hover:bg-pink/20 hover:text-pink hover:scale-105 hover:-translate-y-1 cursor-pointer`}>
                <a href={link.href} className='block lg:inline-block w-full py-1.5 lg:py-0'>{link.name}</a></li>
              );
          })}
        </ul>
