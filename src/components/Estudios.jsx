...
export default function Estudios() {
    ...
    const getButtonClass = (year) => {
        ...
        return `flex items-center justify-center font-bold text-base md:text-lg gap-1 rounded-xl py-2 px-4 shadow-lg border-2  cursor-pointer transition-all duration-300 ease-in-out
    ...
    return (
        <section className='font-family bg-peach-soft dark:bg-dark-bg-soft dark:text-slate-200 flex flex-col justify-center items-center gap-4 pb-8 scroll-mt-20 transition-colors duration-300' id='estudios'>
            ...
            <div className='flex flex-col gap-6 max-w-4xl'>
                ...
                <div className=' hidden md:flex justify-center items-center gap-4'>
                     {yearsList.map((op) => (
                                <button key={op.value} onClick={() => setSelectedYear(op.value)} className={getButtonClass(op.value)}>{op.label !== 'Todos' && <LuGraduationCap className='text-xl' />}{op.label}</button>
                            ))}
                </div>
                <div className='px-5 sm:px-8 md:px-0 grid grid-cols-1 md:grid-cols-2 gap-6'>
                    ...
                    {filteredStudies.map((study) => {
                        // Bootcamp (color verde azulado), En curso (color rosa), Demás (color gris)
                        const stateStudy = study.status === 'En curso';
                        const isBootcamp = study.tag === 'Bootcamp';
                        // Ajustamos el hover según el borde de cada tarjeta
                        ...
                        return (
                            <div key={study.id} className={`relative min-w-75 flex flex-col bg-peach border-2 rounded-xl py-4 px-5 sm:px-8 shadow-xl gap-3 dark:bg-dark-bg-strong transition-transform duration-300 hover:scale-105 ${stateStudy ? 'border-pink/60 dark:border-pink' : isBootcamp ?  'border-blue-green' : 'border-grey-medium/90 dark:border-grey-medium/50'}`}>
                                {/* Mostramos el icono de Ojo cuando haya certificado disponible */}
                               { study.certificateUrl && ( <a href={study.certificateUrl} target="_blank"
                                rel="noopener noreferrer" 
                                className={`absolute right-4 top-4 p-2 text-slate-400 border border-slate-200/60 dark:border-grey-strong/20 bg-white/35 dark:bg-grey-strong dark:text-grey-medium rounded-full transition-all duration-200 cursor-pointer ${hoverStyles}`}><FaRegEye aria-label="Ver certificado" className="text-xl" />
                                </a>
                            )}
                                ...
                                <h3 className='pr-8 text-base sm:text-md md:text-lg lg:text-xl font-bold'>{study.title}</h3>
                                <h4 className='text-md md:text-lg'>{study.institution}</h4>
